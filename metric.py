...
def cs_divergence(pred_1, pred_2, pred_3, target, epsilon=1e-8):
    """
...
class Loss(nn.Module):
    ...
    # 定义 KL 散度损失函数，用于模态对齐
    def kl_divergence_loss(self, pred, target):
        """
        计算KL散度损失。
        :param pred: 预测的相似度矩阵，形状为 (batch_size, batch_size)。
        :param target: 目标相似度矩阵，形状为 (batch_size, batch_size)。
        :return: KL散度损失。
        """
    ...
    def create_target_matrix(self, labels):
        ...
        labels_reshape = torch.reshape(labels, (B, 1))
        labels_dist = labels_reshape - labels_reshape.t()  # (B,B)
        labels_mask = (labels_dist == 0).float()
        return labels_mask.float()

    def compute_recall_at_k(self, similarity_scores, labels, k=1):
        """
        计算召回率。
    ...
    def compute_cmcl_loss(self, image_embeddings, text_embeddings, audio_embeddings, labels):
        ...
        # 计算KL散度损失
        #loss = 0
        #loss += self.kl_divergence_loss(S_t2a_pred, S_target) + self.kl_divergence_loss(S_a2t_pred, S_target)
        #loss += self.kl_divergence_loss(S_t2i_pred, S_target) + self.kl_divergence_loss(S_i2t_pred, S_target)
        #loss += self.kl_divergence_loss(S_a2i_pred, S_target) + self.kl_divergence_loss(S_i2a_pred, S_target)
        
        # 计算CS散度
        loss = 0
        loss = cs_divergence(S_t2a_pred, S_t2i_pred, S_a2i_pred, S_target) + cs_divergence(S_a2t_pred, S_i2t_pred, S_i2a_pred, S_target)
